"""
================================================================================
AI SHELL - MONGODB UTILITIES MODULE
================================================================================
This module provides MongoDB integration for logging and session management.

WHY MONGODB (NoSQL) FOR THIS USE CASE:
=======================================
MongoDB is used here for LOGGING and SESSION DATA, not for command validation.
This architectural decision is based on key NoSQL strengths:

1. SCHEMA FLEXIBILITY:
   - Log documents can have varying fields based on context
   - New fields can be added without schema migrations
   - Different command types may produce different metadata

2. DOCUMENT-ORIENTED STORAGE:
   - Each interaction is a self-contained document
   - Natural fit for event/log data with nested structures
   - Easy to store execution errors, stack traces, etc.

3. HIGH WRITE THROUGHPUT:
   - Logging requires frequent inserts with minimal latency
   - MongoDB excels at write-heavy workloads
   - No complex transactions needed for append-only logs

4. HORIZONTAL SCALABILITY:
   - If logs grow large, MongoDB can scale out easily
   - SQLite would require manual sharding or archival

5. TIME-SERIES DATA:
   - Session logs are inherently time-ordered
   - MongoDB has native support for time-series collections
   - Easy to query logs by time range

WHAT STAYS IN SQLITE (Relational):
==================================
- Command rules (structured, normalized data)
- Risk levels (enforced by CHECK constraints)
- Approved templates (referential integrity needed)
- These require ACID transactions and foreign keys

Author: AI Shell Project
Database: MongoDB (ai_shell_logs)
================================================================================
"""
